...
export class LoginComponent {
  ...
  async forgotPassword() {
    ...
    if (!email || !this.loginForm.get("email")?.valid) {
      this.errorMsg = "Please enter a valid email address to reset your password."; // Display error for invalid email
      return;
    }

    try {
      await this._loginSvc.forgotPassword(email); // Call the forgot password service
      alert("Password reset email sent! Please check your inbox."); // Notify user of successful email
    } catch (error) {
